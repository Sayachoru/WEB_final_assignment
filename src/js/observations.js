...
function titleCase(s) {
  ...
  arrWords.forEach((element) => {
    var arrLetters = element.split("");
    arrLetters[0] = arrLetters[0].toUpperCase();
    results.push(arrLetters + " ");
  });
...
function transformObservations(observations) {
  return observations.map(function (observation) {
    ...
    coords[0] = observation.geojson.coordinates[0];
    coords[1] = observation.geojson.coordinates[1];
    ...
    var newArr = {
      ...
      isExtinct: observation.taxon.extinct,
...
function getAllObservations() {
  const filter = filterObservations(window.data.results);
  const transform = transformObservations(filter);
